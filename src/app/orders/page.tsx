  ...
  <main className="flex-1 app-container w-full px-5 sm:px-8 lg:px-10 pt-12 sm:pt-14 pb-24 sm:pb-28 text-[13px]">
        ...
        <div>
          {filtered.map(o=>{
            ...
            return (
              <div key={o.id} className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-5 py-7 border-b last:border-b-0">
                ...
                <div className="flex-1 min-w-0 pt-0.5 order-2 sm:order-none">
                  <h3 className="text-[13px] font-semibold mb-1">{o.product}</h3>
                  <p className="text-[11px] text-gray-500">Order {tabMeta.verb} on - <span className="font-medium">{formatOrderDate(o.placed)}</span></p>
                </div>
                <div className="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-4 shrink-0 order-1 sm:order-none">
                  <p className="text-[13px] font-semibold">${o.price}</p>
                  <Link href={`/orders/${o.id}`} className="inline-flex items-center justify-center h-9 px-5 rounded-full border border-gray-400 text-[11px] hover:bg-gray-50 w-auto">View Details →</Link>
                </div>
              </div>
            );
          })}
